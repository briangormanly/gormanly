...
exports.getAllTags = async function ( req, res ) {
    ...
    if( tags.length > 0 ) {
        res.set( "x-gormanly-message-title", "Success" );
        res.set( "x-gormanly-message-detail", "Returned all tags" );
        res.status( 200 ).json( tags );
    }
    else {
        res.set( "x-gormanly-message-title", "Not Found" );
        res.set( "x-gormanly-message-detail", "No tags were found meeting the query criteria" );
...
exports.getTagById = async function( req, res ) {
    ...
    if( tag ) {
        res.set( "x-gormanly-message-title", "Success" );
        res.set( "x-gormanly-message-detail", "Returned tag" );
...
exports.saveTag = async function( req, res, redirect ) {
    ...
    if( redirect ) {
        ...
        if ( existingTagName ) {
            ...
            res.set( "x-gormanly-message-title", "Success" );
            res.set( "x-gormanly-message-detail", "Updated existing Tag by Tag name" );
            res.status( 200 ).send( JSON.stringify( tag ) );
        }
        else {
            res.setHeader( 'Content-Type', 'application/json' );
            res.set( "x-gormanly-message-title", "Success" );
            res.set( "x-gormanly-message-detail", "Created new tag" );
...
exports.getTaggedEntity = async function( req, res ) {
    ...
    if( tags.length > 0 ) {
        res.set( "x-gormanly-message-title", "Success" );
        res.set( "x-gormanly-message-detail", "Returned all tags associated with requested entity" );
        res.status( 200 ).json( tags );
    }
    else {
        res.set( "x-gormanly-message-title", "Not Found" );
...
exports.deleteTagged = async function( req, res ) {
    ...
    if( success ) {
        res.set( "x-gormanly-message-title", "Success" );
        res.set( "x-gormanly-message-detail", "Tag association deleted" );
